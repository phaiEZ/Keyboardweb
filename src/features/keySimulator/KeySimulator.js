...
import { keycapscolor } from "./../keyModules/keycaps.js";
...
function KeySimulator({ currentTheme, theme }) {
  ...
  const handleSwitchChange = (e) => {
    ...
    toast.show(
      `Switch sound changed to ${keySounds[e.target.value].caption} ✔️`,
      {
        timeout: 3000,
        pause: false,
        delay: 0,
  ...
  const handleLayoutChange = (e) => {
    ...
    keycontainer.current.focus();
    console.log(keyPresets[e.target.value].kle);
  };

  const handleKeycapsChange = (e) => {
    //dispatch(parseKLE(keyPresets[e.target.value].kle));
    layoutselect.current.blur();
    keycontainer.current.focus();
        ...
        >
          <div className={styles.selectarea}>
            ...
            <div className={styles.selectcol}>
              ...
              >
                {keyPresets.map((preset, index) => {
                  return (
                    <option value={index} key={preset.key}>
                      {preset.caption}
                    </option>
                  );
                })}
            ...
            <div className={styles.selectcol}>
              <button
                type="button"
                class="inline-block px-6 py-2.5  bg-red-500 text-white font-bold text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-700 active:shadow-lg transition duration-150 ease-in-out"
              >
                Order
              </button>
            </div>
